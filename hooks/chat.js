  ...
  useEffect(() => {
    if (incomingMessage) {
      ...
      if (incomingMessage.message || incomingMessage.media) {
        const message = incomingMessage.media ? 'Multimedia 📁' : incomingMessage.message
        setChats((prevChats) =>
          prevChats.map((chat) =>
            chat.id === incomingMessage.idChat
              ? {
                  ...
                  unread: incomingMessage.sender === 0 && incomingMessage.idChat !== idChat ? chat.unread + 1 : chat.unread,
                  last_date: Date.now(),
                  isActive: incomingMessage.isActive === 0 ? 0 : 1,
                }
              : chat,
          ),
        )
      ...
      if (incomingMessage.action === 'message_read') {
        setChats((prevChats) => prevChats.map((chat) => (chat.id === incomingMessage.idChat ? { ...chat, unread: 0 } : chat)))
      }
    }
  }, [incomingMessage])
