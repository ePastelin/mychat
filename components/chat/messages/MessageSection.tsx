  ...
  return (
    <div className='messageLayout'>
      {messages.map((msg, index) => (
        <div
          key={msg.id}
          className={`flex ${msg.sender === 1 ? 'justify-end' : 'justify-start'} mb-2`}
        ...
        >
          ...
          >
            {msg.media ? <ShowMessage msg={msg} /> : msg.message}
            ...
            <div className='bottom-0 right-0 text-xs text-gray-300 mr-2 flex items-center space-x-1 self-end'>
              ...
              {msg.sender === 1 && (
                <div className='flex items-center'>
                  <span className={`${msg.status === 'read' ? 'text-cyan-200' : 'text-senderMessage'}`}>{getStatusIcon(msg.status)}</span>
                </div>
              )}
            </div>
          </div>
