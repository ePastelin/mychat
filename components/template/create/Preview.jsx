export default function Preview({ headerText, headerExamples, bodyText, bodyExamples, footerText, footerExamples, buttons }) {
  ...
  return (
    <section className='bg-chatBackground h-auto mt-6 py-6 rounded-2xl flex justify-center items-center'>
      <div className=' w-64 bg-blue-500 h-auto py-5 px-3 rounded-3xl shadow-dropshadow-message'>
        <p className='text-white text-sm mb-4'>{replaceVariables(headerText, headerExamples)}</p>

        <p className='text-white text-sm mb-4'>{replaceVariables(bodyText, bodyExamples)}</p>

        <p className='text-white text-sm mb-4'>{replaceVariables(footerText, footerExamples)}</p>
